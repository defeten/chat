...
export function Settings() {
  ...
  return (
    <div className="mx-auto flex w-full grow flex-col justify-center gap-y-4 rounded-md p-2 text-stone-300 md:w-1/2">
      ...
      <div className="flex items-center justify-between">
        ...
        <Switch.Root
          ...
          onChange={({ currentTarget }) =>
            setTime(currentTarget.dataset.state === "checked")
          }
          className="relative h-6 w-10 cursor-default rounded-full bg-stone-800 outline-none data-[state=checked]:bg-emerald-800"
        >
          <Switch.Thumb className="shadow-blackA4 block size-5 translate-x-0.5 rounded-full bg-stone-400 transition-transform duration-200 will-change-transform data-[state=checked]:translate-x-4.5 data-[state=checked]:bg-stone-200" />
        </Switch.Root>
      </div>
      <div className="flex items-center justify-between">
      ...
      <div className="flex items-center justify-between">
        ...
        <Switch.Root
          id="unfocusaftersend"
          onChange={({ currentTarget }) =>
            setUnfocusInput(currentTarget.dataset.state === "checked")
          }
          className="relative h-6 w-10 cursor-default rounded-full bg-stone-800 outline-none data-[state=checked]:bg-emerald-800"
        >
          <Switch.Thumb className="shadow-blackA4 block size-5 translate-x-0.5 rounded-full bg-stone-400 transition-transform duration-200 will-change-transform data-[state=checked]:translate-x-4.5 data-[state=checked]:bg-stone-200" />
